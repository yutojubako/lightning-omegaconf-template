...
spec = importlib.util.spec_from_file_location(
    "test_instantiate_module", Path(__file__).parent.parent / "src" / "utils" / "instantiate.py"
)
...
def test_instantiate_class() -> None:
    ...
    cfg = OmegaConf.create({"_target_": "tests.helpers.instantiate_targets.Example", "value": 3})
...
def test_instantiate_function() -> None:
    ...
    cfg = OmegaConf.create({"_target_": "tests.helpers.instantiate_targets.add", "a": 2, "b": 5})
    result = instantiate(cfg)
    assert result == 7
...
def test_instantiate_overrides() -> None:
    ...
    cfg = OmegaConf.create({"_target_": "tests.helpers.instantiate_targets.Example", "value": 1})
...
def test_instantiate_invalid_module() -> None:
    ...
    cfg = OmegaConf.create({"_target_": "nonexistent.module.Class"})
    with pytest.raises(
        ModuleNotFoundError,
...
def test_instantiate_invalid_attribute() -> None:
    ...
    cfg = OmegaConf.create({"_target_": "tests.helpers.instantiate_targets.NonExistentClass"})
    with pytest.raises(
        AttributeError,
...
def test_instantiate_target_without_dots() -> None:
    ...
    cfg = OmegaConf.create({"_target_": "dict"})
    result = instantiate(cfg)
    assert result == {}
